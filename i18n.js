...
(function () {
  const T = [
    ...
    [`Dành cho đối tác đã hợp tác cùng DCC: chỉ cần tải hồ sơ ứng viên lên ngay tại đây. Bạn không phải tạo tài khoản — chúng tôi chỉ cần biết hồ sơ này do đối tác nào gửi để tiếp nhận và phản hồi sớm nhất.`, `For partners already working with DCC: simply upload candidate profiles right here. No account needed — we only need to know which partner sent the profile so we can receive it and respond as soon as possible.`, `Für Partner, die bereits mit DCC zusammenarbeiten: Laden Sie Bewerberprofile einfach direkt hier hoch. Kein Konto nötig — wir müssen nur wissen, welcher Partner das Profil gesendet hat, um es entgegenzunehmen und schnellstmöglich zu antworten.`],
    ...
    [`Điền tên đối tác để DCC biết hồ sơ do ai gửi, rồi đính kèm file hồ sơ ứng viên (CV, bằng cấp, giấy tờ). DCC sẽ tiếp nhận và liên hệ lại với bạn.`, `Enter the partner name so DCC knows who sent the profile, then attach the candidate's files (CV, certificates, documents). DCC will receive it and get back to you.`, `Geben Sie den Partnernamen an, damit DCC weiß, wer das Profil gesendet hat, und hängen Sie dann die Bewerberdateien an (Lebenslauf, Zeugnisse, Dokumente). DCC nimmt es entgegen und meldet sich bei Ihnen.`],
    ...
    [`🔒 Email và số điện thoại chỉ DCC nắm để liên hệ lại với bạn — sẽ không xuất hiện trong hồ sơ chia sẻ cho đối tác tại Đức.`, `🔒 Your email and phone are kept by DCC only to contact you back — they will not appear in the profiles shared with partners in Germany.`, `🔒 E-Mail und Telefonnummer werden nur von DCC zur Kontaktaufnahme verwendet — sie erscheinen nicht in den Profilen, die mit Partnern in Deutschland geteilt werden.`],
