...
(function () {
  const T = [
    ...
    [`Gửi hồ sơ ứng viên về DCC`, `Send candidate profiles to DCC`, `Bewerberprofile an DCC senden`],
    [`Dành cho tất cả đối tác của DCC: chỉ cần đăng nhập tài khoản đối tác là tải hồ sơ ứng viên lên được ngay. Bạn không phải nhập lại tên — hệ thống tự nhận biết hồ sơ do đối tác nào gửi để DCC tiếp nhận và phản hồi sớm nhất.`, `For all DCC partners: just log in to your partner account to upload candidate profiles right away. No need to re-enter your name — the system automatically knows which partner sent the profile so DCC can receive it and respond as soon as possible.`, `Für alle DCC-Partner: Melden Sie sich einfach in Ihrem Partner-Konto an, um Bewerberprofile sofort hochzuladen. Sie müssen Ihren Namen nicht erneut eingeben — das System erkennt automatisch, welcher Partner das Profil gesendet hat, damit DCC es entgegennehmen und schnellstmöglich antworten kann.`],
    [`Tải hồ sơ lên`, `Upload documents`, `Unterlagen hochladen`],
    [`Đăng nhập để gửi hồ sơ`, `Log in to send a profile`, `Anmelden, um Profil zu senden`],
    [`Vui lòng đăng nhập tài khoản đối tác`, `Please log in to your partner account`, `Bitte in Ihrem Partner-Konto anmelden`],
    [`Đối tác đăng nhập để DCC biết hồ sơ do ai gửi — bạn không phải nhập lại tên mỗi lần.`, `Partners log in so DCC knows who sent the profile — no need to re-enter your name each time.`, `Partner melden sich an, damit DCC weiß, wer das Profil gesendet hat — Sie müssen Ihren Namen nicht jedes Mal erneut eingeben.`],
    [`Đăng nhập & gửi hồ sơ`, `Log in & send profile`, `Anmelden & Profil senden`],
    [`đổi tài khoản`, `switch account`, `Konto wechseln`],
    [`Liên hệ DCC`, `Contact DCC`, `DCC kontaktieren`],
    [`Upload hồ sơ`, `Upload documents`, `Unterlagen-Upload`],
    [`Đối tác tải hồ sơ ứng viên lên`, `Partners upload candidate profiles`, `Partner laden Bewerberprofile hoch`],
    [`Đăng nhập tài khoản đối tác rồi đính kèm file hồ sơ ứng viên (CV, bằng cấp, giấy tờ). DCC tự biết hồ sơ do đối tác nào gửi và sẽ liên hệ lại với bạn.`, `Log in to your partner account, then attach the candidate's files (CV, certificates, documents). DCC automatically knows which partner sent the profile and will get back to you.`, `Melden Sie sich in Ihrem Partner-Konto an und hängen Sie dann die Bewerberdateien an (Lebenslauf, Zeugnisse, Dokumente). DCC erkennt automatisch, welcher Partner das Profil gesendet hat, und meldet sich bei Ihnen.`],
    [`Form đối tác gửi hồ sơ ứng viên`, `Partner candidate-profile submission form`, `Partner-Formular zur Bewerberübermittlung`],
    [`Tên đối tác gửi *`, `Sending partner name *`, `Name des sendenden Partners *`],
    [`Họ tên ứng viên`, `Candidate full name`, `Name des Bewerbers`],
    [`Email đối tác`, `Partner email`, `Partner-E-Mail`],
    [`SĐT / Zalo đối tác`, `Partner phone / Zalo`, `Partner-Telefon / Zalo`],
    [`Ngành / nghề ứng viên`, `Candidate field / occupation`, `Berufsfeld des Bewerbers`],
    [`File hồ sơ`, `Profile files`, `Bewerbungsdateien`],
    [`Gửi hồ sơ`, `Send profile`, `Profil senden`],
    [`Đính kèm CV, bằng cấp, giấy tờ của ứng viên. Tối đa 5 file, tổng dưới ~3MB. File lớn hơn vui lòng nén .zip hoặc liên hệ DCC để gửi qua kênh khác.`, `Attach the candidate's CV, certificates and documents. Up to 5 files, total under ~3MB. For larger files, please zip them or contact DCC to send via another channel.`, `Hängen Sie Lebenslauf, Zeugnisse und Dokumente des Bewerbers an. Bis zu 5 Dateien, insgesamt unter ~3 MB. Größere Dateien bitte als .zip packen oder DCC für einen anderen Versandweg kontaktieren.`],
    [`🔒 Email và số điện thoại sẽ được che bớt (vd 09••••78) ở cả bản nội bộ lẫn bản gửi đối tác tại Đức — khi cần DCC liên hệ trực tiếp với bạn để lấy đầy đủ.`, `🔒 Email and phone will be partially masked (e.g. 09••••78) in both the internal copy and the copy shared with partners in Germany — when needed, DCC contacts you directly for the full details.`, `🔒 E-Mail und Telefon werden sowohl in der internen Kopie als auch in der für Partner in Deutschland geteilten Kopie teilweise maskiert (z. B. 09••••78) — bei Bedarf kontaktiert DCC Sie direkt für die vollständigen Angaben.`],
